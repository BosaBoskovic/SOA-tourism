...
func TestAMQPHeaders_RoundTripsTraceContext(t *testing.T) {
	...
	// (see the same comment in observability.go's initTracer) - so this
	// test sets the same W3C composite propagator main() sets at startup,
	// otherwise Inject/Extract below would silently do nothing.
...
func TestInjectAMQPHeaders_NilHeaders_AllocatesTable(t *testing.T) {
	headers := InjectAMQPHeaders(context.Background(), nil)
	if headers == nil {
		t.Fatal("expected InjectAMQPHeaders to allocate a table instead of returning nil")
	}
}
